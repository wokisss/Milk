...
.result-container {
  ...
  left: 0;
  z-index: 999;
  padding-top: 60rpx; /* 预留顶部空间，无需调整 */
  box-sizing: border-box;
}

/* 返回按钮样式：关键调整 top 值，往下移动到 80rpx（原30rpx） */
.back-btn {
  position: absolute;
  top: 80rpx; /* 从30rpx调整到80rpx，往下移动50rpx */
  left: 30rpx;
  z-index: 1000;
}
...
.back-icon {
  font-size: 48rpx;
  color: #fff;
  background-color: rgba(0, 0, 0, 0.5); /* 加深背景色，更醒目 */
  width: 60rpx;
  height: 60rpx;
  border-radius: 50%;
  display: inline-flex;
  justify-content: center;
  align-items: center;
  box-shadow: 0 2rpx 8rpx rgba(0, 0, 0, 0.3); /* 加轻微阴影，更有质感 */
}
